...
/// <summary>
/// Instância do Plano que está sendo visto
/// </summary>
public class PlaneVisualizer : MonoBehaviour {
    private DetectedPlane m_DetectedPlane;

    //Váriaveis auxiliares para verificar se o plano não foi modificado e ficar atualizando o mesh a cada interação
    ...
    /// </summary>
    public void Update() {
        //Se não fui inicializado 
        if (m_DetectedPlane == null) {
            return;
        }
        //Se eu me tornei parte de outro plano
        else if (m_DetectedPlane.SubsumedBy != null) {
            Destroy(gameObject);
            return;
        }
        //Se estou sendo detectado pela câmera, me exiba
        else if (m_DetectedPlane.TrackingState != TrackingState.Tracking) {
            m_MeshRenderer.enabled = false;
            return;
        }

        //Exibe a Mesh se for permitido
        m_MeshRenderer.enabled = GameLogic.Instance.CanSeePlane;
